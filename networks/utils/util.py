...
def upsample_mesh(vertices, normals, faces, displacement_map, texture_map, dense_template):
    ...
    return dense_vertices, dense_faces #dense_colors
...
def Ccode2dict(coarse_code):
    codedict = {}
    codedict['shape'] = coarse_code[:, :100]
    codedict['exp'] = coarse_code[:, 100:150]
    codedict['pose'] = coarse_code[:, 150:156]

    codedict['tex'] = coarse_code[:, 156:206]
    codedict['lights'] = coarse_code[:, 206:233].reshape(-1,9,3)
    codedict['cam'] = coarse_code[:, 233:236]
...
def compute_texture_map(source_img, target_mesh_v, vertex_normals, camera_params, texture_data):
    '''
    ...
    x_coords = texture_data.get('x_coords').astype('int')
    y_coords = texture_data.get('y_coords').astype('int')
    valid_pixel_ids = texture_data.get('valid_pixel_ids').astype('int')
    valid_pixel_3d_faces = torch.from_numpy(texture_data.get('valid_pixel_3d_faces').astype('int')).to(torch.long).cuda()
    valid_pixel_b_coords = \
        torch.from_numpy(texture_data.get('valid_pixel_b_coords').astype('float32')).cuda()
    ...
    proj_2d_points_normalized = batch_orth_proj(pixel_3d_points, camera_params)[:, :, :2]
    ...
    texture_grid[:, y_coords[valid_pixel_ids], x_coords[valid_pixel_ids], :] = \
    ...
    pixel_3d_normals = \
        vertex_normals[:, valid_pixel_3d_faces[:, 0], :] * valid_pixel_b_coords[:, 0][None:, None] +\
    ...
    texture_vis_mask[:, :, y_coords[valid_pixel_ids], x_coords[valid_pixel_ids]] = \
...
def save_tensor_img(tensor_img, save_path):
...
def save_tensor_img_cv(tensor_img, save_path):
    np_img = (tensor_img.detach().cpu().numpy().transpose(1,2,0).copy()*255)[:,:,[2,1,0]]
    np_img = np.minimum(np.maximum(np_img, 0), 255).astype(np.uint8)
